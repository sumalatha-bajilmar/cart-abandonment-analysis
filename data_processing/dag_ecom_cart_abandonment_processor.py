...
Description: End-to-end analytics pipeline resolving parameters through AWS SSM.
             ...
             window-reset aggregation rules, and stores enriched metrics in destination S3.
"""
...
def ecom_cart_abandonment_pipeline():
    ...
    def aggregate_and_segment_data(df_cart: pd.DataFrame, df_customers: pd.DataFrame, logical_date=None, **context):
        ...
        # 5. Compute true quantities (Treating 'remove' as a baseline subtractor)
        valid_window_df["calculated_qty"] = np.where(
            valid_window_df["activity_type"] == "remove",
            -valid_window_df["quantity"],
            valid_window_df["quantity"]
        )
        valid_window_df["product_total_price"] = valid_window_df["calculated_qty"] * valid_window_df["price_per_unit"]

        # 6. Aggregate to calculate the final volumes per product, then global user totals
        product_summary = (
            valid_window_df.groupby(["user_id", "product_id"])
            .agg(final_quantity=("calculated_qty", "sum"), product_cart_price=("product_total_price", "sum"))
            .reset_index()
        )
        user_summary = (
        ...
        conditions = [
            ...
            (user_summary["total_quantity"] > 1)
        ]
        choices = ["BucketA", "BucketB", "Bucket_Other"]
        ...
        # Resolves /airflow/variables/s3_target_bucket out of AWS SSM at runtime
        target_bucket = Variable.get("s3_target_bucket")
        execution_date_str = logical_date.strftime("%Y-%m-%d")
        target_key = f"aggregated-cart-metrics/{execution_date_str}/customer_segments.parquet"
    ...
    # TaskFlow API links execution sequence based on dependency variables automatically
